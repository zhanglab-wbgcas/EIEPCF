...
def GENIE3_single(expr_data, output_idx, input_idx, tree_method, K, ntrees):
    ...
    if tree_method == 'RF':
        treeEstimator = RandomForestRegressor(n_estimators=ntrees, max_features=max_features, random_state=123)
    elif tree_method == 'ET':
        treeEstimator = ExtraTreesRegressor(n_estimators=ntrees, max_features=max_features, random_state=123)
